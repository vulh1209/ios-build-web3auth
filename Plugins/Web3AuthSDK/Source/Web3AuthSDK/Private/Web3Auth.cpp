...
FString AWeb3Auth::encryptPrivateKey(FString msgString,FString keyString) {
	...
	if( StringToBytes(msgString, Blob, Size)) { 
...
FString AWeb3Auth::decryptPrivateKey(FString encryptString,FString keyString) {
	
	ANSICHAR* Key = TCHAR_TO_ANSI(*keyString);
	if(encryptString.IsEmpty()) return encryptString;  
	
	uint8* Blob; 
	uint32 Size; 
	Size = encryptString.Len()/2;
	// Size = Size - (FAES::AESBlockSize - (Size % FAES::AESBlockSize));
	Blob = new uint8[Size]; 
	if( FString::ToHexBlob(encryptString,Blob,Size)) { 
		FAES::DecryptData(Blob,Size,Key); 
		encryptString = BytesToString(Blob, Size);
		delete Blob; 
...
FString AWeb3Auth::createAESKey() {
	uint8* Blob = new uint8[32]; 
	StringToBytes(FString::FromInt(FMath::Rand()),Blob,32);
	return FString::FromHexBlob(Blob,32); 
}
