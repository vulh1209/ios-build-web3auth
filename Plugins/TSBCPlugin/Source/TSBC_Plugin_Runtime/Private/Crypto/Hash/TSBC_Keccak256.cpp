...
CTSBC_Keccak256::CTSBC_Keccak256(const EBits Bits)
{
...
void CTSBC_Keccak256::ProcessBlock(const void* Data)
{
...
void CTSBC_Keccak256::Add(const void* Data, size_t NumBytes)
{
...
void CTSBC_Keccak256::ProcessBuffer()
{
...
FString CTSBC_Keccak256::GetHash()
{
    // Save hash state
...
FString CTSBC_Keccak256::KeccakFromString(const FString& Text, const bool& bIsHex)
{
    Reset();
    ...
    if(bIsHex)
    {
        ...
        const auto Length = HexToBytes(Text, Data);
        Add(Data, Length);
    }
    else
    {
        const ANSICHAR* SingleByteArray = TCHAR_TO_ANSI(*Text);
...
FString CTSBC_Keccak256::KeccakFromBytes(const TArray<uint8>& Bytes)
{
    Reset();
